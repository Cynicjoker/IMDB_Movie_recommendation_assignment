...
-- 20 null values in country

select count(*) as counter_nulls
from movie
where worlwide_gross_income is null;

-- 3724 null values in worlwide_gross_income
...
select country, count(id) as count_of_movies
from movie
where country = 'USA' or country = 'India' and year = 2019
group by country;

-- (P.S.) many movies produced in multiple countries which can found using where like '%India%' or '%USA%'
...
select count(movie_id) as solo_genre 
from filter;
...
select  median_rating, count(r.movie_id) as no_of_movies
from movie m 
inner join ratings r on
m.id=r.movie_id
...
with top_genre as
(
select genre, count(g.movie_id) as mcount
from genre as g
...
where r.avg_rating > 8 and genre 
in
(
select genre
from top_genre
...
(
 ...
 from
	names n inner join director_mapping d on n.id=d.name_id
	inner join ratings r on d.movie_id=r.movie_id
	inner join movie m on m.id=r.movie_id
	inner join avg_inter_days a on a.name_id=d.name_id
